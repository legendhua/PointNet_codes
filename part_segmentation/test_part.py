...
for idx in range(len(oid2cpid)):
    ...
    object2setofoid[objid].append(idx)   #每一类object标号和所包含的part组成dict
...
on2oid = {objcats[i]:i for i in range(len(objcats))}  #每一类的类别号和标号组成dict
fin.close()
...
#add zgh 20171002
part_belongto_cat_file=os.path.join(hdf5_data_dir,'part_belong_to_object.json')
part_belongto_cat=json.load(open(part_belongto_cat_file,'r'))
#end
...
def predict():
    ...
    with tf.Session(config=config) as sess:
        ...
        # add 2018-0103
        fiou = open(os.path.join(output_dir,'out.txt'),'w') #增加记录每个测试数据的IOU的文件
        # end
        ...
        total_per_part_iou=np.zeros((NUM_PART_CATS)).astype(np.float32)#add zgh 20171002
        ...
        for shape_idx in range(len_pts_files):
            ...
            for oid in iou_oids:
                ...
                if n_union == 0:
                    total_iou += 1
                    part_iou += 1#add zgh 20171004
                    iou_log += '_1\n'
                else:
                    total_iou += n_intersect * 1.0 / n_union
                    part_iou += n_intersect * 1.0 / n_union#add zgh 20171004
                    iou_log += '_'+str(n_intersect * 1.0 / n_union)+'\n'
                total_per_part_iou[oid]+=part_iou #add zgh
        ...
        #add zgh 20171002
        printout(flog,'\t show the part IOU')
        for part_idx in range(NUM_PART_CATS):
            if total_per_cat_seen[part_belongto_cat[part_idx]]>0:
                printout(flog,'\t part '+str(part_idx)+' of '+str(objnames[part_belongto_cat[part_idx]])+' IOU: '+\
                str(total_per_part_iou[part_idx]/total_per_cat_seen[part_belongto_cat[part_idx]]))
        #end zgh        
