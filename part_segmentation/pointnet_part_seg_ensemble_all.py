...
#shallow1
def get_first_model(point_cloud, input_label, is_training, cat_num, part_num, \
    ...
    net_transformed_max = tf_util.max_pool2d(net_transformed, [num_point,1], padding='VALID', scope='maxpool4')
    ...
    concat = tf.concat(axis=3, values=[expand,out1, out2, out3,out1_max, out4,out5])
...
def get_second_model(point_cloud, input_label, is_training, cat_num, part_num, \
    ...
    out4 = tf_util.conv2d(net_transformed, 512, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv4', bn_decay=bn_decay)
    out4_max = tf_util.max_pool2d(out4, [num_point,1], padding='VALID', scope='maxpool5')
    out4_max= tf.tile(out4_max, [1, num_point, 1, 1])
    ...
    expand = tf.tile(out_max, [1, num_point, 1, 1])
    #concat = tf.concat(axis=3, values=[expand, out1, out2, out3, out4, out5])#modify by zgh
    concat = tf.concat(axis=3, values=[expand, out1, out2, out2_max, out3, out4,out5])
...
#shallow3
def get_third_model(point_cloud, input_label, is_training, cat_num, part_num, \
		batch_size, num_point, weight_decay, bn_decay=None):
    """ ConvNet baseline, input is BxNx3 gray image """
    end_points = {}
    ...
    expand = tf.tile(out_max, [1, num_point, 1, 1])
    #concat = tf.concat(axis=3, values=[expand, out1, out2, out3, out4, out5])#modify by zgh
    concat = tf.concat(axis=3, values=[expand, out1, out2, out3, out3_max, out4, out5])
...
#shallow4
def get_forth_model(point_cloud, input_label, is_training, cat_num, part_num, \
		batch_size, num_point, weight_decay, bn_decay=None):
    """ ConvNet baseline, input is BxNx3 gray image """
    end_points = {}
    ...
    expand = tf.tile(out_max, [1, num_point, 1, 1])
    #concat = tf.concat(axis=3, values=[expand, out1, out2, out3, out4, out5])#modify by zgh
    concat = tf.concat(axis=3, values=[expand, out3,out4_max, out4,out5])
