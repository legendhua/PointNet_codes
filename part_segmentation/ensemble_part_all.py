...
import provider
import pointnet_part_seg_ensemble_all as model

parser = argparse.ArgumentParser()
parser.add_argument('--first_model_path', default='ensemble_log/shallow1/epoch_200.ckpt', help='Model checkpoint path')
parser.add_argument('--second_model_path', default='ensemble_log/shallow2/epoch_190.ckpt', help='Model checkpoint path')
parser.add_argument('--third_model_path', default='ensemble_log/shallow3/epoch_200.ckpt', help='Model checkpoint path')
parser.add_argument('--forth_model_path', default='ensemble_log/shallow4/epoch_200.ckpt', help='Model checkpoint path')
FLAGS = parser.parse_args()


# DEFAULT SETTINGS
pretrained_first_model_path = FLAGS.first_model_path # os.path.join(BASE_DIR, './pretrained_model/model.ckpt')
pretrained_second_model_path = FLAGS.second_model_path # os.path.join(BASE_DIR, './pretrained_model/model.ckpt')
pretrained_third_model_path = FLAGS.third_model_path # os.path.join(BASE_DIR, './pretrained_model/model.ckpt')
pretrained_forth_model_path = FLAGS.forth_model_path # os.path.join(BASE_DIR, './pretrained_model/model.ckpt')
...
def predict_first_model():
    ...
    with tf.device('/gpu:'+str(gpu_to_use)):
        ...
        # simple model
        pred, seg_pred, end_points = model.get_first_model(pointclouds_ph, input_label_ph, \
    ...
    with tf.Session(config=config) as sess:
        ...
        printout(flog, 'Loading model %s' % pretrained_first_model_path)
        saver.restore(sess, pretrained_first_model_path)
        printout(flog, 'First Model restored.')
        ...
        #add 2018-0109
        first_model_seg = np.zeros((1,50))
        #end
        len_pts_files = len(pts_files)
        printout(flog,"----------first model-----------")
        for shape_idx in range(len_pts_files):
            ...
            #seg_pred_res  shape:1x3000x50
            label_pred_val = np.argmax(label_pred_val[0, :])
            seg_pred_res = seg_pred_res[0, ...]
            #seg_pred_res  shape:3000x50
            #add 2018-0109
            first_model_seg = np.row_stack((first_model_seg,seg_pred_res))#为了实验增加一个全零行
            #end
    first_model_seg = np.delete(first_model_seg,0,axis=0)
    #最后数据维度：（2874*3000）×50

    return first_model_seg
            
def predict_second_model():
    is_training = False
    ...
    with tf.device('/gpu:'+str(gpu_to_use)):
        ...
        # simple model
        pred, seg_pred, end_points = model.get_second_model(pointclouds_ph, input_label_ph, \
                cat_num=NUM_OBJ_CATS, part_num=NUM_PART_CATS, is_training=is_training_ph, \
                batch_size=batch_size, num_point=point_num, weight_decay=0.0, bn_decay=None)
    ...
    with tf.Session(config=config) as sess:
        ...
        # Restore variables from disk.
        printout(flog, 'Loading model %s' % pretrained_second_model_path)
        saver.restore(sess, pretrained_second_model_path)
        printout(flog, 'Second Model restored.')
        ...
        #add 2018-0109
        second_model_seg = np.zeros((1,50))
        #end
        len_pts_files = len(pts_files)
        printout(flog,"----------second model-----------")
        for shape_idx in range(len_pts_files):
            ...
            seg_pred_res = seg_pred_res[0, ...]
            #add 2018-0109
            second_model_seg = np.row_stack((second_model_seg,seg_pred_res))
            #end
    second_model_seg = np.delete(second_model_seg,0,axis=0)
    return second_model_seg

def predict_third_model():
    is_training = False
    ...
    with tf.device('/gpu:'+str(gpu_to_use)):
        ...
        # simple model
        pred, seg_pred, end_points = model.get_third_model(pointclouds_ph, input_label_ph, \
                cat_num=NUM_OBJ_CATS, part_num=NUM_PART_CATS, is_training=is_training_ph, \
                batch_size=batch_size, num_point=point_num, weight_decay=0.0, bn_decay=None)
    ...
    with tf.Session(config=config) as sess:
        ...
        # Restore variables from disk.
        printout(flog, 'Loading model %s' % pretrained_third_model_path)
        saver.restore(sess, pretrained_third_model_path)
        printout(flog, 'Third Model restored.')
        ...
        #add 2018-0109
        third_model_seg = np.zeros((1,50))
        #end
        len_pts_files = len(pts_files)
        printout(flog,"----------third model-----------")
        for shape_idx in range(len_pts_files):
            ...
            seg_pred_res = seg_pred_res[0, ...]
            #add 2018-0109
            third_model_seg = np.row_stack((third_model_seg,seg_pred_res))
            #end
    third_model_seg = np.delete(third_model_seg,0,axis=0)
    return third_model_seg

def predict_forth_model():
    is_training = False
    ...
    with tf.device('/gpu:'+str(gpu_to_use)):
        ...
        # simple model
        pred, seg_pred, end_points = model.get_forth_model(pointclouds_ph, input_label_ph, \
                cat_num=NUM_OBJ_CATS, part_num=NUM_PART_CATS, is_training=is_training_ph, \
                batch_size=batch_size, num_point=point_num, weight_decay=0.0, bn_decay=None)
    ...
    with tf.Session(config=config) as sess:
        ...
        # Restore variables from disk.
        printout(flog, 'Loading model %s' % pretrained_forth_model_path)
        saver.restore(sess, pretrained_forth_model_path)
        printout(flog, 'Forth Model restored.')
        ...
        #add 2018-0109
        forth_model_seg = np.zeros((1,50))
        #end
        len_pts_files = len(pts_files)
        printout(flog,"----------forth model-----------")
        for shape_idx in range(len_pts_files):
            ...
            seg_pred_res = seg_pred_res[0, ...]
            #add 2018-0109
            forth_model_seg = np.row_stack((forth_model_seg,seg_pred_res))
            #end
    forth_model_seg = np.delete(forth_model_seg,0,axis=0)
    return forth_model_seg

def ensemble_part():                
    with tf.Graph().as_default():
        first_seg_val = predict_first_model()
    with tf.Graph().as_default():
        second_seg_val = predict_second_model()
    with tf.Graph().as_default():
        third_seg_val = predict_third_model()
    with tf.Graph().as_default():
        forth_seg_val = predict_forth_model()
    seg_pred_res_val = (first_seg_val+second_seg_val+third_seg_val+forth_seg_val)/4
    ...
    for shape_idx in range(len_pts_files):
        ...
        seg_pred_res = seg_pred_res_val[(shape_idx*3000):((shape_idx+1)*3000),:]
